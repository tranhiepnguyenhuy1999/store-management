...
function CustomerTable2({field, form, data}) {
    ...
    const columns = [
        ...
        },
        {
          ...
          key: 'price',
          render: (text,record)=>{
            return <InputNumber min={0} max={100000} value={record.price} onChange={(value)=>onChangePrice(value, record.id)} />
          }
        },
        {
            title: 'Total',
            key: 'total',
            render: (text, record) =>{
                return (record.price * record.number *2)
            }
        },
        {
            title: '',
